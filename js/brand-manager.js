...
import { BrandsDB, FontsDB, AssetsDB, ColorsDB } from './db.js'
...
export class BrandManager {
  ...
  getCurrentBrandId() { return this._currentBrandId }
  ...
  async renderPalette(container, onColorClick, onColorDelete) {
    ...
    if (!brand || !brand.palette?.length) {
      container.innerHTML = '<span style="font-size:11px;color:var(--text-disabled);">Sem cores salvas</span>'
      return
    }
  ...
  async renderLogos(container, onLogoClick) {
    container.innerHTML = ''
    const brand = await this.getCurrentBrand()
    const logos = brand ? await this.getBrandAssets(brand.id, 'logo') : await this.getAllLogos()
  ...
  async renderFonts(container, onFontClick) {
    ...
    const brand = await this.getCurrentBrand()
    const fonts = brand
      ? [...await this.getBrandFonts(brand.id), ...await this.getGlobalFonts()]
      : await this.getGlobalFonts()

    if (!fonts.length) {
      container.innerHTML = '<span style="font-size:11px;color:var(--text-disabled);">Nenhuma fonte salva</span>'
      return
    }

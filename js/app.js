...
import { initDB, PresetsDB, ColorsDB } from './db.js'
import { FORMATS, FORMAT_GROUPS, getFormat } from './formats.js'
import { CanvasEngine, createDefaultState, makeBadgeLayer, makeHeadlineLayer, makeSubLayer, makeTextLayer, makeImageLayer, makeShapeLayer } from './canvas-engine.js'
...
const GOOGLE_FONTS = [
  'Inter', 'Roboto', 'Montserrat', 'Poppins', 'Raleway',
  'Playfair+Display', 'Oswald', 'Lato', 'Nunito', 'Open+Sans',
  'Source+Sans+3', 'Ubuntu', 'Josefin+Sans', 'DM+Sans',
]
...
class App {
  ...
  _loadFonts() {
    ...
    link.rel = 'stylesheet'
    const families = GOOGLE_FONTS.map(f => `family=${f}:wght@300;400;500;600;700;800;900`).join('&')
  ...
  _refreshPropertiesPanel(layer) {
    ...
    // Hide all
    ;[textSection, posSection, badgeSection, imageSection, shapeSection].forEach(el => {
  ...
  _fillTextControls(layer) {
    ...
    if (layer.subtype === 'badge') {
      ...
      this._setSwatch('prop-badge-bg-swatch', layer.badgeBg === 'transparent' ? 'transparent' : layer.badgeBg)
  ...
  _fillPositionControls(layer) {
    ...
    this._setVal('prop-width', layer.width === 'auto' ? 'auto' : layer.width?.toFixed(1) ?? 'auto')
  }
  ...
  _wireBgControls() {
    ...
    // Solid color
    document.getElementById('prop-bg-color-swatch')?.addEventListener('click', () => {
    ...
    // Gradient
    document.getElementById('prop-grad-from-swatch')?.addEventListener('click', () => {
      const bg = this.canvas.getState().background
      this._openPicker(bg.gradient?.from ?? '#000', (rgba) => {
        const grad = { ...this.canvas.getState().background.gradient, from: rgba }
    ...
    document.getElementById('prop-grad-to-swatch')?.addEventListener('click', () => {
      const bg = this.canvas.getState().background
      this._openPicker(bg.gradient?.to ?? '#1a1a2e', (rgba) => {
        const grad = { ...this.canvas.getState().background.gradient, to: rgba }
        this.canvas.updateBackground({ gradient: grad })
    ...
    document.getElementById('prop-grad-angle')?.addEventListener('input', (e) => {
      const angle = parseInt(e.target.value)
      document.getElementById('prop-grad-angle-val').textContent = angle + '°'
      const grad = { ...this.canvas.getState().background.gradient, angle }
      this.canvas.updateBackground({ gradient: grad })
      this._updateGradientBar()
    })

    document.getElementById('prop-grad-type')?.addEventListener('change', (e) => {
      const grad = { ...this.canvas.getState().background.gradient, type: e.target.value }
      this.canvas.updateBackground({ gradient: grad })
    })
    ...
    document.getElementById('btn-bg-image')?.addEventListener('click', () => {
      this._openFilePicker('image/*', async (file) => {
        ...
        document.querySelectorAll('.bg-type-btn').forEach(b => b.classList.toggle('active', b.dataset.bgType === 'image'))
        document.querySelectorAll('.bg-panel').forEach(p => p.classList.add('hidden'))
        document.getElementById('bg-panel-image')?.classList.remove('hidden')
      })
    })

    document.getElementById('prop-bg-image-size')?.addEventListener('change', (e) => {
  ...
  _wirePropertyInputs() {
    ...
    const onInput = (id, apply) => {
      ...
      if (!el) return
      const event = el.tagName === 'TEXTAREA' || el.tagName === 'SELECT' ? 'change' : 'input'
    ...
    panel.querySelector('#prop-content')?.addEventListener('focus', () => this.canvas.snapshot())
    ...
    panel.querySelector('#prop-font-size')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { fontSize: parseFloat(e.target.value) || layer.fontSize })
    })
    panel.querySelector('#prop-font-size')?.addEventListener('focus', () => this.canvas.snapshot())
    ...
    panel.querySelector('#prop-font-weight')?.addEventListener('change', (e) => {
      ...
      if (!layer) return
      this.canvas.snapshot()
      this.canvas.updateLayer(layer.id, { fontWeight: parseInt(e.target.value) })
    })
    ...
    panel.querySelector('#prop-line-height')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { lineHeight: parseFloat(e.target.value) || 1 })
    })
    panel.querySelector('#prop-line-height')?.addEventListener('focus', () => this.canvas.snapshot())
    ...
    panel.querySelector('#prop-letter-spacing')?.addEventListener('focus', () => this.canvas.snapshot())

    // Opacity
    panel.querySelector('#prop-opacity')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { opacity: parseInt(e.target.value) / 100 })
    })
    panel.querySelector('#prop-opacity')?.addEventListener('focus', () => this.canvas.snapshot())
    ...
    const bindPos = (id, prop) => {
      panel.querySelector(`#${id}`)?.addEventListener('input', (e) => {
        ...
        if (!isNaN(val) || val === 'auto') this.canvas.updateLayer(layer.id, { [prop]: val })
      })
      panel.querySelector(`#${id}`)?.addEventListener('focus', () => this.canvas.snapshot())
    }
    bindPos('prop-x', 'x')
    bindPos('prop-y', 'y')
    ...
    panel.querySelector('#prop-badge-border-radius')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { badgeBorderRadius: parseFloat(e.target.value) || 0 })
    })
    panel.querySelector('#prop-badge-padding-x')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { badgePaddingX: parseFloat(e.target.value) || 0 })
    })
    panel.querySelector('#prop-badge-padding-y')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { badgePaddingY: parseFloat(e.target.value) || 0 })
    })
    ...
    panel.querySelector('#prop-shape-height')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { height: parseFloat(e.target.value) || 0.5 })
    })
    panel.querySelector('#prop-shape-radius')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { borderRadius: parseFloat(e.target.value) || 0 })
    })
    ...
    panel.querySelector('#prop-img-height')?.addEventListener('input', (e) => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.updateLayer(layer.id, { height: parseFloat(e.target.value) || 40 })
    })
    ...
    panel.querySelector('#btn-duplicate-layer')?.addEventListener('click', () => {
      const layer = this.canvas.getSelectedLayer()
      if (!layer) return
      this.canvas.snapshot()
    ...
    this.brands.on('requestAddLogo', () => {
      this._openFilePicker('image/*', async (file) => {
        ...
        if (brandId) {
          ...
          await this.brands.addAsset({ name: file.name, type: 'logo', data: b64, mimeType: file.type })
        }
        await this._refreshSidebar()
  ...
  /* ── Format modal ─────────────────────────────────────── */
  _wireFormatModal() {
    document.getElementById('btn-close-format-modal')?.addEventListener('click', () => {
  ...
  _openFormatModal() {
    ...
    FORMAT_GROUPS.forEach(group => {
      ...
      group.formats.forEach(fmtId => {
        ...
        const maxW = 38, maxH = 46
        const ratio = fmt.width / fmt.height
        let tw = maxW, th = maxH
        if (ratio >= 1) { th = Math.round(maxW / ratio) }
        else { tw = Math.round(maxH * ratio) }
  ...
  _wireExportModal() {
    ...
    document.getElementById('btn-export-all-formats')?.addEventListener('click', async () => {
  ...
  _openAddFontModal() {
    ...
    if (!name) return
    const brandId = this.brands.getCurrentBrandId()
    this.brands.addFontToBrand(brandId, { name, family: name, source: 'google' }).then(async () => {
      await this._refreshSidebar()
  ...
  _setAlign(id, align) {
    document.querySelectorAll(`[data-align-group="${id}"] .align-btn`).forEach(btn => {

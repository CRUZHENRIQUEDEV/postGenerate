...
## Templates & Layouts System — Futura Implementação
...
|---|---|---|---|
| **Alto** | Setas para mover camadas, toggle lock/visibility, contador chars, atalhos doc | Arrastar slides, alinhamento/distribuição, JPEG/WebP export, eyedropper | Drag/resize no canvas, edição inline de texto, multi-select |
| **Médio** | Histórico de cores, modal de atalhos, variante IA | Animações de saída, copiar/colar estilo, A/B IA, PDF export | Grupos de camadas, crop de imagem |
| **Baixo** | Modo claro, guias de área segura, `prefers-reduced-motion` | Tons automáticos de paleta, versionamento de preset | Streaming IA, Worker para export |
